...
This is the canonical public agent-safety policy for **sonus-auris**. It applies directly to this repository and is the minimum policy every repository in the organization must mirror in its root `AGENTS.md` or replace with a stricter equivalent.
...
Never print, log, commit, paste into issues, or expose tokens, credentials, private keys, personal data, production data, recordings, transcripts, biometric data, or secret-bearing environment variables. Use placeholders in examples and redact diagnostics.
